...
/**
 * REST controller for the Agent Skills module.
 *
 * @package OxyAI
 */
...
/**
 * A thin, agent-skills-specific facade over the shared Discovery/
 * Validation/Generation engines, mirroring `RobotsController` exactly
 * under its own `/agent-skills/*` namespace.
 */
final class AgentSkillsController
{
    private const GENERATOR_ID = 'agent-skills';
    private const RESOURCE_ID = 'agent-skills-registry';
    ...
    public function validate(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if (!isset($map[self::RESOURCE_ID])) {
            return new WP_REST_Response(
                ['success' => false, 'message' => 'Agent Skills registry has not been discovered.'],
                404
            );
        }
