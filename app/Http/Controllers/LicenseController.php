...
/**
 * REST controller for the License module.
 *
 * @package OxyAI
 */
...
/**
 * A thin, license-specific facade over the shared Discovery/
 * Validation/Generation engines, mirroring `RobotsController` exactly
 * under its own `/license/*` namespace.
 */
final class LicenseController
{
    private const GENERATOR_ID = 'license';
    private const RESOURCE_ID = 'license-status';
    ...
    public function validate(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if (!isset($map[self::RESOURCE_ID])) {
            return new WP_REST_Response(
                ['success' => false, 'message' => 'License status has not been discovered.'],
                404
            );
        }
