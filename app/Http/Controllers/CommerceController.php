...
/**
 * REST controller for the Commerce module.
 *
 * @package OxyAI
 */
...
/**
 * A thin, commerce-specific facade over the shared Discovery/
 * Validation/Generation engines, mirroring `RobotsController` exactly
 * under its own `/commerce/*` namespace.
 */
final class CommerceController
{
    private const GENERATOR_ID = 'commerce';
    private const RESOURCE_ID = 'commerce-status';
    ...
    public function validate(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if (!isset($map[self::RESOURCE_ID])) {
            return new WP_REST_Response(
                ['success' => false, 'message' => 'Commerce status has not been discovered.'],
                404
            );
        }
