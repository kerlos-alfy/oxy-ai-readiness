...
/**
 * REST controller for the Robots module.
 *
 * @package OxyAI
 */
...
/**
 * A thin, robots-specific facade over the shared Discovery/Validation/
 * Generation engines — the "Full Robots module... wired through
 * Discovery→Generation→Validation→Scoring" the Phase 8 roadmap row
 * describes, exposed under its own `/robots/*` namespace per the exit
 * criterion ("/robots/* REST fully functional") rather than the
 * generic `/discovery`/`/validation`/`/generation` routes. Gated
 * behind `manage_options`, same interim default as every other
 * controller so far.
 */
final class RobotsController
{
    private const GENERATOR_ID = 'robots';
    private const RESOURCE_ID = 'robots-txt';
    ...
    public function validate(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if (!isset($map[self::RESOURCE_ID])) {
            return new WP_REST_Response(
                ['success' => false, 'message' => 'robots.txt has not been discovered.'],
                404
            );
        }
