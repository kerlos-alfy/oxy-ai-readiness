...
/**
 * REST controller for the MCP module.
 *
 * @package OxyAI
 */
...
/**
 * A thin, mcp-specific facade over the shared Discovery/Validation/
 * Generation engines, mirroring `RobotsController` exactly under its
 * own `/mcp/*` namespace.
 */
final class McpController
{
    private const GENERATOR_ID = 'mcp';
    private const RESOURCE_ID = 'mcp-server-card';
    ...
    public function validate(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if (!isset($map[self::RESOURCE_ID])) {
            return new WP_REST_Response(
                ['success' => false, 'message' => 'MCP server card has not been discovered.'],
                404
            );
        }
