...
/**
 * REST controller for the Content Signals module.
 *
 * @package OxyAI
 */
...
/**
 * A thin, content-signals-specific facade over the shared Discovery/
 * Validation/Generation engines, mirroring `RobotsController` exactly
 * under its own `/content-signals/*` namespace.
 */
final class ContentSignalsController
{
    private const GENERATOR_ID = 'content-signals';
    private const RESOURCE_ID = 'content-signals';
    ...
    public function validate(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if (!isset($map[self::RESOURCE_ID])) {
            return new WP_REST_Response(
                ['success' => false, 'message' => 'Content signals have not been discovered.'],
                404
            );
        }
