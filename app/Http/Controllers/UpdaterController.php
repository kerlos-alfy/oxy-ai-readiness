...
/**
 * REST controller for the Updater module.
 *
 * @package OxyAI
 */
...
/**
 * A thin, updater-specific facade over the shared Discovery/
 * Validation/Generation engines, mirroring `RobotsController` exactly
 * under its own `/updater/*` namespace.
 */
final class UpdaterController
{
    ...
    public function validate(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if (!isset($map[self::RESOURCE_ID])) {
            return new WP_REST_Response(
                ['success' => false, 'message' => 'Updater status has not been discovered.'],
                404
            );
        }
