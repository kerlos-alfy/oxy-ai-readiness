...
/**
 * REST controller for the API Catalog module.
 *
 * @package OxyAI
 */
...
/**
 * A thin, api-catalog-specific facade over the shared Discovery/
 * Validation/Generation engines, mirroring `RobotsController` exactly
 * under its own `/api-catalog/*` namespace.
 */
final class ApiCatalogController
{
    private const GENERATOR_ID = 'api-catalog';
    private const RESOURCE_ID = 'api-catalog';
    ...
    public function validate(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if (!isset($map[self::RESOURCE_ID])) {
            return new WP_REST_Response(
                ['success' => false, 'message' => 'API catalog has not been discovered.'],
                404
            );
        }
