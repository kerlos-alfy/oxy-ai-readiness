...
/**
 * REST controller for the Headers module.
 *
 * @package OxyAI
 */
...
/**
 * A thin, headers-specific facade over the shared Discovery/
 * Validation/Generation engines, mirroring `RobotsController` exactly
 * under its own `/headers/*` namespace.
 */
final class HeadersController
{
    private const GENERATOR_ID = 'headers';
    private const RESOURCE_ID = 'http-headers';
    ...
    public function validate(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if (!isset($map[self::RESOURCE_ID])) {
            return new WP_REST_Response(
                ['success' => false, 'message' => 'HTTP headers have not been discovered.'],
                404
            );
        }
