...
/**
 * REST controller for the LLMS module.
 *
 * @package OxyAI
 */
...
/**
 * A thin, llms-specific facade over the shared Discovery/Validation/
 * Generation engines, mirroring `RobotsController` exactly under its
 * own `/llms/*` namespace.
 */
final class LlmsController
{
    private const GENERATOR_ID = 'llms';
    private const RESOURCE_ID = 'llms-txt';
    ...
    public function validate(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if (!isset($map[self::RESOURCE_ID])) {
            return new WP_REST_Response(
                ['success' => false, 'message' => 'llms.txt has not been discovered.'],
                404
            );
        }
