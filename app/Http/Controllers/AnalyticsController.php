...
/**
 * A thin, analytics-specific facade over the shared Discovery/
 * Validation/Generation engines, mirroring `RobotsController` exactly
 * under its own `/analytics/*` namespace.
 */
final class AnalyticsController
{
    private const GENERATOR_ID = 'analytics';
    private const RESOURCE_ID = 'analytics-summary';
    ...
    public function validate(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if (!isset($map[self::RESOURCE_ID])) {
            return new WP_REST_Response(
                ['success' => false, 'message' => 'Analytics summary has not been discovered.'],
                404
            );
        }
