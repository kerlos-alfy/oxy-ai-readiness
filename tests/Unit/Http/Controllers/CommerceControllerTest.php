...
final class CommerceControllerTest extends TestCase
{
    private function makeController(): CommerceController
    {
        ...
        $discovery->registerProvider('commerce', $discoveryProvider);

        $validator = Mockery::mock(ValidatorInterface::class);
        $validator->allows('validate')->andReturn(
            new ValidationResult('commerce-status', 'commerce', ValidationStatus::Pass, 'ok', 0.1)
        );
        $validation = new ValidationService();
        $validation->registerValidator('commerce', $validator);
        ...
        $generator->allows('resourceId')->andReturn('commerce-status');
        $generator->allows('generate')->andReturn('{"woocommerce_active":false}');
        $generation->registerGenerator('commerce', $generator);

        return new CommerceController($discovery, $validation, $generation);
    }
