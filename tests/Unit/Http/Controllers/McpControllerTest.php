...
{
    private function makeController(): McpController
    {
        ...
        $discovery->registerProvider('mcp', $discoveryProvider);

        $validator = Mockery::mock(ValidatorInterface::class);
        $validator->allows('validate')->andReturn(
            new ValidationResult('mcp-server-card', 'mcp', ValidationStatus::Pass, 'ok', 0.1)
        );
        $validation = new ValidationService();
        $validation->registerValidator('mcp', $validator);
        ...
        $generator->allows('resourceId')->andReturn('mcp-server-card');
        $generator->allows('generate')->andReturn('{"name":"Test"}');
        $generation->registerGenerator('mcp', $generator);

        return new McpController($discovery, $validation, $generation);
    }
