...
final class LicenseControllerTest extends TestCase
{
    private function makeController(): LicenseController
    {
        ...
        $discovery->registerProvider('license', $discoveryProvider);

        $validator = Mockery::mock(ValidatorInterface::class);
        $validator->allows('validate')->andReturn(
            new ValidationResult('license-status', 'license', ValidationStatus::Pass, 'ok', 0.1)
        );
        $validation = new ValidationService();
        $validation->registerValidator('license', $validator);
        ...
        $generator->allows('resourceId')->andReturn('license-status');
        $generator->allows('generate')->andReturn('{"tier":"free"}');
        $generation->registerGenerator('license', $generator);

        return new LicenseController($discovery, $validation, $generation);
    }
