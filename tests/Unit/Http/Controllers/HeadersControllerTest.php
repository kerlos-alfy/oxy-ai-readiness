...
final class HeadersControllerTest extends TestCase
{
    private function makeController(): HeadersController
    {
        ...
        $discovery->registerProvider('headers', $discoveryProvider);

        $validator = Mockery::mock(ValidatorInterface::class);
        $validator->allows('validate')->andReturn(
            new ValidationResult('http-headers', 'headers', ValidationStatus::Pass, 'ok', 0.1)
        );
        $validation = new ValidationService();
        $validation->registerValidator('headers', $validator);
        ...
        $generator->allows('id')->andReturn('headers');
        $generator->allows('resourceId')->andReturn('http-headers');
        $generator->allows('generate')->andReturn("X-Content-Type-Options: nosniff\n");
        $generation->registerGenerator('headers', $generator);

        return new HeadersController($discovery, $validation, $generation);
    }
