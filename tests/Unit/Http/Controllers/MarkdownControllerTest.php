...
{
    private function makeController(): MarkdownController
    {
        ...
        $discovery->registerProvider('markdown', $discoveryProvider);

        $validator = Mockery::mock(ValidatorInterface::class);
        $validator->allows('validate')->andReturn(
            new ValidationResult('markdown-negotiation', 'markdown', ValidationStatus::Pass, 'ok', 0.1)
        );
        $validation = new ValidationService();
        $validation->registerValidator('markdown', $validator);
        ...
        $generator->allows('id')->andReturn('markdown');
        $generator->allows('resourceId')->andReturn('markdown-negotiation');
        $generator->allows('generate')->andReturn("Content-Type: text/markdown\n");
        $generation->registerGenerator('markdown', $generator);

        return new MarkdownController($discovery, $validation, $generation);
    }
