...
{
    private function makeController(): LlmsController
    {
        ...
        $discovery->registerProvider('llms', $discoveryProvider);

        $validator = Mockery::mock(ValidatorInterface::class);
        $validator->allows('validate')->andReturn(
            new ValidationResult('llms-txt', 'llms', ValidationStatus::Pass, 'ok', 0.1)
        );
        $validation = new ValidationService();
        $validation->registerValidator('llms', $validator);
        ...
        $generator->allows('id')->andReturn('llms');
        $generator->allows('resourceId')->andReturn('llms-txt');
        $generator->allows('generate')->andReturn("# Site\n\n> Description\n");
        $generation->registerGenerator('llms', $generator);

        return new LlmsController($discovery, $validation, $generation);
    }
