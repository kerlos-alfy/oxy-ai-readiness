...
{
    private function makeController(): RobotsController
    {
        ...
        $discovery->registerProvider('robots', $discoveryProvider);
        ...
        $generator->allows('id')->andReturn('robots');
        $generator->allows('resourceId')->andReturn('robots-txt');
        $generator->allows('generate')->andReturn("User-agent: *\nAllow: /\n");
        $generation->registerGenerator('robots', $generator);

        return new RobotsController($discovery, $validation, $generation);
    }
