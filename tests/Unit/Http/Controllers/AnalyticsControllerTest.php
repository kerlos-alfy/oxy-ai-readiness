...
final class AnalyticsControllerTest extends TestCase
{
    private function makeController(): AnalyticsController
    {
        ...
        $discovery->registerProvider('analytics', $discoveryProvider);

        $validator = Mockery::mock(ValidatorInterface::class);
        $validator->allows('validate')->andReturn(
            new ValidationResult('analytics-summary', 'analytics', ValidationStatus::Pass, 'ok', 0.1)
        );
        $validation = new ValidationService();
        $validation->registerValidator('analytics', $validator);
        ...
        $generator->allows('resourceId')->andReturn('analytics-summary');
        $generator->allows('generate')->andReturn('{"period":"daily"}');
        $generation->registerGenerator('analytics', $generator);

        return new AnalyticsController($discovery, $validation, $generation);
    }
