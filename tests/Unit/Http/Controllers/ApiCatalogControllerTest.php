...
{
    private function makeController(): ApiCatalogController
    {
        ...
        $discovery->registerProvider('api-catalog', $discoveryProvider);

        $validator = Mockery::mock(ValidatorInterface::class);
        $validator->allows('validate')->andReturn(
            new ValidationResult('api-catalog', 'api-catalog', ValidationStatus::Pass, 'ok', 0.1)
        );
        $validation = new ValidationService();
        $validation->registerValidator('api-catalog', $validator);
        ...
        $generator->allows('resourceId')->andReturn('api-catalog');
        $generator->allows('generate')->andReturn('{"namespace":"oxy-ai/v1","routes":[]}');
        $generation->registerGenerator('api-catalog', $generator);

        return new ApiCatalogController($discovery, $validation, $generation);
    }
